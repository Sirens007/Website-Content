...
## 2. 希尔排序
1959年，**唐纳德·希尔（Donald Shell）**<font style="color:rgb(15, 17, 21);"> 提出了一个改进思路：</font>

先让数组**基本有序**，再用插入排序完成最终排序。

希尔排序即，缩小增量排序，其**具体思想**为：

1. 先选定一个整数 **gap**（称为增量），把待排序文件中的所有记录分成 **gap 组**，所有距离为 gap 的记录分在同一组内；
2. 对**每一组内的记录进行直接插入排序**；
3. 然后**缩小增量**（如 `gap = gap / 2），**重复**上述分组和排序的工作；
4. 当 **gap = 1** 时，所有记录在同一组内，此时再执行一次直接插入排序，整个序列就排好序了。
...
循环次数为 n-1 次；每次 siftDown 的时间复杂度为**O(log n)**（堆的高度），则**总时间复杂度 = O(n log n)**。

两部分的**总时间复杂度**则为：**O(N logN)**  
**空间复杂度为 O(1)**

## 5. 冒泡排序
冒泡排序过程：

1. 相邻元素两两比较，如果前面比后面大，就交换
2. 每一轮结束后，当前未排序部分的最大值会“沉”到末尾
3. 重复直到全部有序
...
3. **cur 指针从 left+1 开始向右遍历**，寻找比基准值小的元素：
    1. 若 `array[cur] < array[left]`，说明找到比基准小的值
    2. 先将 `prev++`，扩大至小于基准的区域
    3. 若`array[prev] != array[cur]`，则交换 `array[prev]`<font style="color:rgb(15, 17, 21);"> 与 </font>`array[cur]`，将小值移到左边
4. **重复步骤 3**，知道 cur 指针遍历完整个区间
5. **最后将基准值填入 prev 位置**，交换 `array[prev]`与 `array[left]`，返回 prev 作为基准最终位置
...
1. 三数取中是为了**选择一个更好的基准值**，以提高快速排序的效率。在快速排序中，选择一个合适的基准值是非常重要的，它决定了每次分割的平衡性。
2. 如果每次选择的基准值都是最左边或最右边的元素，那么在某些情况下，快速排序的效率可能会降低。例如，**当待排序序列已经有序时，如果每次选择的基准值都是最左边或最右边的元素，那么每次分割得到的两个子序列的长度差可能会非常大**，导致递归深度增加，快速排序的效率降低。
3. 而通过三数取中的优化，可以选择一个更好的基准值，使得每次分割得到的两个子序列的长度差更小，从而提高快速排序的效率；也就是**尽量满足之前所说的全满二叉树**，而不是单支二叉树
...
1. 先判断**当前数组长度**是否≤1，如果是则直接返回（递归出口）；
2. 若长度大于 1，则找到数组中间位置，将数组分成**左半部分**和**右半部分**；
3. 对左半部分**递归调用并排序**（重复 1~3 步，直到左半部分为单独个体）
4. 对右半部分**递归调用并排序**（重复 1~3 步，直到右半部分为单独个体）
5. 左右两部分都已有序后，**新建一个临时数组**；
6. **同时遍历左右两部分**：比较两个部分的当前元素，将较小的那个放入临时数组，并移动对应部分的指针；
7. 重复第 6 步，直到某一部分的所有元素都已被放入临时数组；
8. 将另一部分剩余的所有元素依次放入临时数组；
9. 将临时数组中的元素复制回原数组对应的位置；
10. 返回上一层的递归，继续合并更大的有序片段，直到整个数组完全有序；
